...
if not getrawmetatable or not setreadonly or not setrawmetatable then
...
local base= UIBase.new():setLabel("gfnfchgxfb cwel")
do
	local tabList= base:newTabList()

	local main= tabList:newTab("Combat"):intoSections()
	do
...
if hookmetamethod then
	local oldAntiBan
	oldAntiBan = hookmetamethod(game, "__namecall", LPH_NO_VIRTUALIZE(function(self, ...)
		local method = getnamecallmethod()
		if (method == "FireServer" or method == "fireServer") and ({...})[1] == "\000\000" then
			--LocalPlayer:Kick("Game attempted to ban you, report this to a developer on our discord")
			return
		end
		return oldAntiBan(self, ...)
	end))
end

local clientFighterScript = LocalPlayer.PlayerScripts:WaitForChild("Modules"):WaitForChild("ClientReplicatedClasses"):WaitForChild("ClientFighter")

local PlayerDataController = require(LocalPlayer.PlayerScripts.Controllers:WaitForChild("PlayerDataController"))
if not PlayerDataController._elements_loaded then
	PlayerDataController:WaitUntilLoaded()
	task.wait(8)
end

local ClientFighter = require(clientFighterScript)
local ClientViewModel = require(clientFighterScript:WaitForChild("ClientItem"):WaitForChild("ClientViewModel"))
local FighterController = require(LocalPlayer.PlayerScripts:WaitForChild("Controllers"):WaitForChild("FighterController"))

local CameraController = require(LocalPlayer.PlayerScripts:WaitForChild("Controllers"):WaitForChild("CameraController"))
local GunItemType = require(LocalPlayer.PlayerScripts:WaitForChild("Modules"):WaitForChild("ItemTypes"):WaitForChild("Gun"))

local GameConstants = require(ReplicatedStorage:WaitForChild("Modules"):WaitForChild("CONSTANTS"))
local GameUtility = require(ReplicatedStorage:WaitForChild("Modules"):WaitForChild("Utility"))
...
local function hookSingleMetamethod(instance, mtName, fn)
...
do -- Aimbot stuff
	local function createCircle()
		...
		return screenGui, frame, uiStroke
	end

	local fovGui, fovCircle, fovOutline = createCircle()

	LPH_NO_VIRTUALIZE(function()
		function Aiming.IsVisible(pos, model)
			local raycastParams = RaycastParams.new()
			raycastParams.IgnoreWater = true
			raycastParams.FilterType = Enum.RaycastFilterType.Exclude
			local ignoreList = {Camera, workspace.ViewModels, LocalPlayer.Character}
			for _, player in next, Players:GetPlayers() do
				if player.Character and not Aiming.IsEnemy(player) then
			...
			local raycastResult = workspace.Raycast(workspace, head.Position, pos - head.Position, raycastParams)
			if raycastResult then
				local instanceModel = raycastResult.Instance:FindFirstAncestorWhichIsA("Model")
				return model == instanceModel
			end
			return false
		end

		function Aiming.IsEnemy(player)
			return LocalPlayer:GetAttribute("TeamID") ~= player:GetAttribute("TeamID") and player:GetAttribute("EnvironmentID") and player:GetAttribute("EnvironmentID") == LocalPlayer:GetAttribute("EnvironmentID")
		end

		function Aiming.GetClosestTarget()
			local hitparts = {}
			if base.features["main/silent_aim/hitparts"].value.Head then
				table.insert(hitparts, "Head")
			end
			if base.features["main/silent_aim/hitparts"].value.Body then
				table.insert(hitparts, "Body")
			end
			if #hitparts == 0 then return nil end

			local closestDist, target = math.huge, nil

			for _, player in next, Players:GetPlayers() do
				if player == LocalPlayer then continue end
				if not Aiming.IsEnemy(player) then continue end

				local character = player.Character
				local humanoid = character and character:FindFirstChildOfClass("Humanoid")
				local head = humanoid and character:FindFirstChild("Head")
				local rootPart = head and character:FindFirstChild("HumanoidRootPart")
				if not rootPart then continue end
				if humanoid.Health <= 0 then continue end

				local rootScreenXYZ, onScreen = Camera:WorldToViewportPoint(rootPart.Position)
				local headScreenXYZ, onScreen2 = Camera:WorldToViewportPoint(head.Position)
				if not base.features["main/silent_aim/ignore_fov"].value and not onScreen and not onScreen2 then continue end
				local rootScreenXY = Vector2.new(rootScreenXYZ.X, rootScreenXYZ.Y)
				local headScreenXY = Vector2.new(headScreenXYZ.X, headScreenXYZ.Y)

				local rootDistFromCenter = (rootScreenXY - Camera.ViewportSize / 2).Magnitude
				if (not base.features["main/silent_aim/ignore_fov"].value and rootDistFromCenter > base.features["main/silent_aim/fov"].value) or rootDistFromCenter >= closestDist then continue end
				local headDistFromCenter = (headScreenXY - Camera.ViewportSize / 2).Magnitude

				local hbHead = character:FindFirstChild("HitboxHead")
				local hbBody = character:FindFirstChild("HitboxBody")

				if base.features["main/silent_aim/visible_check"].value then
					if hbHead and not Aiming.IsVisible(hbHead.Position, character) then hbHead = nil end
					if hbBody and not Aiming.IsVisible(hbBody.Position, character) then hbBody = nil end
				end

				if not hbHead and not hbBody then continue end

				local closest
				if #hitparts == 1 then
					closest = hitparts[1] == "Head" and hbHead or hbBody
				else
					if headDistFromCenter < rootDistFromCenter then closest = hbHead else closest = hbBody end
				end

				closestDist = rootDistFromCenter
				target = {Head = hbHead, RootPart = hbBody, Closest = closest}
			end

			return target
		end

		function Aiming.SelectAppropriateTargetPos(target)
			if base.features["main/silent_aim/selection_method"].value == "Random" then
				local preferredHitpart = base.features["main/silent_aim/preferred_hitpart"].value
				local weights = {
					Head = preferredHitpart == "Head" and 4 or 1,
					Body = preferredHitpart == "Body" and 4 or 1
				}
				local totalWeight = weights.Head + weights.Body
				local randomValue = math.random() * totalWeight
				local cumulativeWeight = 0

				for name, hitbox in next, target do
					if name == "Closest" then continue end
					local weight = weights[name] or 0
					cumulativeWeight = cumulativeWeight + weight
					if randomValue <= cumulativeWeight then
						return hitbox.Position + Aiming.CalculateAimOffset(hitbox.Size), hitbox
					end
				end


				if target.Closest then
					return target.Closest.Position + Aiming.CalculateAimOffset(target.Closest.Size), target.Closest
				end
			else
				if target.Closest then
					return target.Closest.Position + Aiming.CalculateAimOffset(target.Closest.Size), target.Closest
		...
		function Aiming.CalculateAimOffset(hitboxSize)
			return base.features["main/silent_aim/randomize_aimpoint"] and Vector3.new(
				random:NextNumber(0, hitboxSize.X / 2) - hitboxSize.X / 2,
				random:NextNumber(0, hitboxSize.Y / 2) - hitboxSize.Y / 2,
				random:NextNumber(0, hitboxSize.Z / 2) - hitboxSize.Z / 2
			) or Vector3.zero
		end
	end)()

	base.features["main/visuals/draw_fov"].changed:Connect(function(state)
		fovGui.Enabled = state
	end)

	if setidentity then
		setidentity(8)
	end
	base.features["main/silent_aim/fov"].changed:Connect(function(fov)
		fovCircle.Size = UDim2.new(0, fov, 0, fov)
	end)
	fovCircle.Size = UDim2.new(0, base.features["main/silent_aim/fov"].value, 0, base.features["main/silent_aim/fov"].value)

	base.features["main/silent_aim/ignore_fov"].changed:Connect(function(state)
		fovGui.Enabled = not state
	end)

	base.features["main/silent_aim/draw_fov/color"].changed:Connect(function(value)
		fovOutline.Color = value.rgb
	end)
end
...
do -- Esp
	...
	local espContainer = Instance.new("ScreenGui", CoreGui)
	...
	local function createHighlight()
		...
		highlight.FillColor = base.features["esp/chams/fill/color/hidden"].value.rgb
		highlight.FillTransparency = 1 - base.features["esp/chams/fill/color/hidden"].value.alpha
		highlight.OutlineColor = base.features["esp/chams/outline/color/hidden"].value.rgb
		highlight.OutlineTransparency = 1 - base.features["esp/chams/outline/color/hidden"].value.alpha
		if setidentity then
			setidentity(8)
		end
	...
	local function createNametag()
		...
		nameTag.AnchorPoint = Vector2.new(0.5, 0.5)
		nameTag.SizeConstraint = Enum.SizeConstraint.RelativeXY
		nameTag.TextScaled = false
		nameTag.TextWrapped = false
		nameTag.Visible = false
		nameTag.Parent = espContainer

		local textSizeConstraint = Instance.new("UITextSizeConstraint")
		textSizeConstraint.MaxTextSize = 13
		textSizeConstraint.MinTextSize = 13
		textSizeConstraint.Parent = nameTag

		-- Create an outline using UIStroke
		local outline = Instance.new("UIStroke")
		outline.Thickness = 1  -- Outline thickness
		outline.Color = Color3.new(0, 0, 0)  -- Black outline color
		outline.Parent = nameTag

		return nameTag
	end

	local function createHealthbar()
		-- Create the outer frame (outline)
		local outerFrame = Instance.new("Frame")
		outerFrame.BackgroundColor3 = Color3.new(0, 0, 0)
		outerFrame.BorderSizePixel = 0
		outerFrame.Parent = espContainer
		...
		healthBar.Parent = outerFrame

		return outerFrame
	end

	LPH_NO_VIRTUALIZE(function()
	local function isPlayerVisible(position, character)
		local raycastParams = RaycastParams.new()
		...
		local raycastResult = workspace:Raycast(cameraPos, position - cameraPos, raycastParams)
	...
	local function calculateStaticBoxBase(screenPoint, depth)
        ...
        width *= viewportSize.X / 1920
        height *= viewportSize.Y / 1080
        ...
        local size = floorVector2(Vector2.new(math.max(width, 4), math.max(height, 6.5)))
        local position = floorVector2(Vector2.new(screenPoint.X - width / 2, screenPoint.Y - height / 2))
	...
	local function playerAdded(player)
		...
		local function characterAdded(character)
			...
			newEspData.character = character
			newEspData.rootPart = rootPart
			newEspData.humanoid = humanoid
			newEspData.alive = true
			...
			newEspData.healthChangeTick = tick()
			newEspData.lastHealth = math.clamp(humanoid.Health, 0, humanoid.MaxHealth)
			newEspData.health = math.clamp(humanoid.Health, 0, humanoid.MaxHealth)

			-- Cleanup on death
			local healthChanged
			healthChanged = humanoid:GetPropertyChangedSignal("Health"):Connect(function()
				newEspData.healthChangeTick = tick()
				newEspData.lastHealth = newEspData.health
				newEspData.health = math.clamp(humanoid.Health, 0, humanoid.MaxHealth)

				if humanoid.Health <= 0 then
					newEspData.alive = false
	...
	do -- Register player added/player removed and add existing players
		for _, player in next, Players:GetPlayers() do
			if player == LocalPlayer then continue end
			playerAdded(player)
		end
	...
	-- Ui handling
	base.features["esp/chams/enabled"].changed:Connect(function(state)
		for _, data in next, espData do
	...
	RunService.PreRender:Connect(function()
		...
		for _, data in next, espData do
			if data.alive then
				...
				local isVisible = Aiming.IsVisible(rootPos, data.character)

				local screenVec3, onScreen = Camera:WorldToViewportPoint(rootPos)
				if not onScreen or not data.alive or not Aiming.IsEnemy(data.player) then
					nametag.Visible = false
					healthBar.Visible = false
					continue
				end
				local screenVec2 = Vector3.new(screenVec3.X, screenVec3.Y)

				local baseSize, basePosition = calculateStaticBoxBase(screenVec2, screenVec3.Z)

				if base.features["esp/nametags/enabled"].value then
					local nametagPosition = Vector2.new(baseSize.X / 2 + basePosition.X, basePosition.Y - math.round(nametag.TextBounds.Y / 2))
					nametag.TextColor3 = isVisible and base.features["esp/nametags/visible"].value.rgb or base.features["esp/nametags/hidden"].value.rgb
					nametag.Position = UDim2.new(0, nametagPosition.X, 0, nametagPosition.Y)
					nametag.Text = base.features["esp/nametags/usedisplayname"].value and data.player.DisplayName or data.player.Name
					nametag.Visible = true
				else
					nametag.Visible = false
				end

				if base.features["esp/healthbars/enabled"].value then
					local health = data.health
					if base.features["esp/healthbars/animhploss"].value then
						local t = math.clamp((tick() - data.healthChangeTick) / base.features["esp/healthbars/animhploss/time"].value, 0, 1)
						local b = data.lastHealth
						local c = health - b
						local d = 1
						health = math.ceil(easeOutCirc(t, b, c, d))
					end

					local healthScale = math.clamp(health / data.humanoid.MaxHealth, 0, 1)


					local barPosition = basePosition - Vector2.new(3 + 4, 0)
					healthBar.Position = UDim2.new(0, barPosition.X, 0, barPosition.Y)
					healthBar.Size = UDim2.new(0, 4, 0, baseSize.Y)

					healthBar.healthBar.Position = UDim2.new(0, 1, 1 - healthScale, 1)
					healthBar.healthBar.Size = UDim2.new(0, 2, healthScale, -2)
					healthBar.healthBar.BackgroundColor3 = base.features["esp/healthbars/color_min"].value.rgb:Lerp(base.features["esp/healthbars/color_max"].value.rgb, healthScale)
				...
				if isVisible then
					highlight.FillColor = base.features["esp/chams/fill/color/visible"].value.rgb
					highlight.FillTransparency = 1 - base.features["esp/chams/fill/color/visible"].value.alpha
					highlight.OutlineColor = base.features["esp/chams/outline/color/visible"].value.rgb
					highlight.OutlineTransparency = 1 - base.features["esp/chams/outline/color/visible"].value.alpha
				else
					highlight.FillColor = base.features["esp/chams/fill/color/hidden"].value.rgb
					highlight.FillTransparency = 1 - base.features["esp/chams/fill/color/hidden"].value.alpha
					highlight.OutlineColor = base.features["esp/chams/outline/color/hidden"].value.rgb
                ...
                highlight.Enabled = base.features["esp/chams/enabled"].value
			else
...
do -- Some misc
	local originalItemDatas = {}
	local function ModifyGunStats()
		local localFighter = FighterController.LocalFighter
		if not localFighter then return end

		for item in next, localFighter:GetEquippedItems() do
			local info = item.Info

			if not originalItemDatas[info.Name] then
				originalItemDatas[info.Name] = table.clone(info)
			end
			local originalItemData = originalItemDatas[info.Name]

			-- No Recoil
			if info.ShootRecoil then
				info.ShootRecoil = base.features["misc/guns/no_recoil"].value and 0 or originalItemData.ShootRecoil
			end

			-- No Spread
			if info.ShootSpread then
				info.ShootSpread = base.features["misc/guns/no_spread"].value and 0 or originalItemData.ShootSpread
			end

            if info.AimSpreadMultiplier then
                info.AimSpreadMultiplier = base.features["misc/guns/no_spread"].value and 0 or originalItemData.AimSpreadMultiplier
            end

			-- Firerate
			if info.ShootCooldown then
				info.ShootCooldown = base.features["misc/guns/no_shoot_cooldown"].value and 0 or originalItemData.ShootCooldown
			end

			-- Straight Projectile
			-- if info.ProjectileSpawnOffset then
			--     info.ProjectileSpawnOffset = ENABLE_STAT_MODIFIER and CFrame.identity or originalItemData.ProjectileSpawnOffset
			-- end

			-- Aim zoom multiplier
			if info.AimFOVOffset then
				info.AimFOVOffset = base.features["misc/guns/aim_fov_mult/enabled"].value and originalItemData.AimFOVOffset * base.features["misc/guns/aim_fov_mult/mult"].value or originalItemData.AimFOVOffset
			end

			-- Infinite Jump
			info.MaxDoubleJumps = base.features["misc/movement/infinite_jump"].value and 9e8 or originalItemData.MaxDoubleJumps

			-- if info.Cooldown then
			--     info.Cooldown = ENABLE_STAT_MODIFIER and 0 or originalItemData.Cooldown
			--     print("util")
			-- end
		end
	end

	FighterController._remote_folder.ObjectChanged.OnClientEvent:Connect(function(v15, v16, ...)
		if type(v15) == "string" then
			local object = FighterController:GetObject(v15)
			if object == FighterController.LocalFighter then
				ModifyGunStats()
			end
		end
	end)

	for _, featureFlag in next, {
		"misc/movement/infinite_jump",
		"misc/guns/no_recoil",
		"misc/guns/no_spread",
		"misc/guns/no_shoot_cooldown",
		"misc/guns/aim_fov_mult/enabled",
		"misc/guns/aim_fov_mult/mult"
	} do
		base.features[featureFlag].changed:Connect(function()
			ModifyGunStats()
		end)
	end

    local o_GetSpread = GameUtility.GetSpread
    GameUtility.GetSpread = function(...)
        if base.features["misc/guns/no_spread"].value then
    ...
    local old = GunItemType._ProjectileEffect
    GunItemType._ProjectileEffect = LPH_NO_VIRTUALIZE(function(v68, projectilePart, v70)
        if base.features["main/projectile_manip/enabled"].value then
            if base.features["main/projectile_manip/method"].value == "Hitscan" then
                local raycastParams = RaycastParams.new()
                raycastParams.FilterType = Enum.RaycastFilterType.Exclude
                ...
                if raycastResult then
                    projectilePart:PivotTo(CFrame.new(raycastResult.Position))
                end
            elseif base.features["main/projectile_manip/method"].value == "Teleport" then
                local targets = Aiming.GetClosestTarget()
				if targets then
					local targetPos, hitbox = Aiming.SelectAppropriateTargetPos(targets)
                    if targetPos then
                        projectilePart.CanCollide = false
                        projectilePart.CanTouch = false
                        projectilePart.CanQuery = false
                        projectilePart:PivotTo(CFrame.new(hitbox.Position))
                        projectilePart.Velocity = Vector3.zero
                    end
                end
            end
        end
        return old(v68, projectilePart, v70)
    end)
end

-- Aimbot hook
local oldNamecall
oldNamecall = hookSingleMetamethod(workspace, "__namecall", LPH_NO_VIRTUALIZE(function(self, ...)
    if base.features["main/silent_aim/enabled"].value and base.features["main/silent_aim/enabled/key"].active then
		local trace = debug.traceback()
		if getnamecallmethod() == "Raycast" and trace:find("ClientFighter") and not trace:find("ReplicateFromServer") then
			local args = {...}
			local origin, direction = args[1], args[2]

			if origin ~= nil and direction ~= nil and math.random(1, 100) <= base.features["main/silent_aim/hitchance"].value then
				local targets = Aiming.GetClosestTarget()
				if targets then
					local targetPos, hitbox = Aiming.SelectAppropriateTargetPos(targets)
					...
					if targetPos then
						...
						pcall(function() game:Raycast() end)
...
-- Common silent flag metamethod hook
-- local oldAntiBan
-- oldAntiBan = hookSingleMetamethod(ReplicatedStorage:WaitForChild("Remotes").Replication.Fighter.UpdateCameraRotation, "__namecall", LPH_NO_VIRTUALIZE(function(self, ...)
-- 	local method = getnamecallmethod()
-- 	if (method == "FireServer" or method == "fireServer") and ({...})[1] == "\000\000" then
...
do
	local canSlideFunction = rawget(ClientFighter, "CanSlide")
	if type(canSlideFunction) == "function" then
		rawset(ClientFighter, "CanSlide", newcclosure(LPH_NO_VIRTUALIZE(function(...)
			if base.features["misc/movement/no_slide_cooldown"].value then
				return true
			else
				return canSlideFunction(...)
			end
		end)))
	end

	-- local applyRecoilFunction = rawget(ClientViewModel, "ApplyRecoil")
	-- if type(applyRecoilFunction) == "function" then
	-- 	print("uwu")
	-- 	local old
	-- 	old = hookfunction(applyRecoilFunction, LPH_NO_UPVALUES(function(...)
	-- 		print("No recoil")
	-- 		if base.features["misc/viewmodel/no_recoil"].value then
	-- 			return
	-- 		else
	-- 			return old(...)
	-- 		end
	...
	task.spawn(LPH_NO_VIRTUALIZE(function()
		...
		local lastWalkSpeed
		while task.wait() do
			if base.features["misc/movement/jump_height/enabled"].value then
				-- apply
				if not lastJumpPower then lastJumpPower = rawget(GameConstants, "BASE_JUMPPOWER") end
				rawset(GameConstants, "BASE_JUMPPOWER", lastJumpPower * base.features["misc/movement/jump_height/mult"].value)
			else
				-- revert
				if lastJumpPower then
					rawset(GameConstants, "BASE_JUMPPOWER", lastJumpPower)
					lastJumpPower = nil
				end
			end

			if base.features["misc/movement/speed_multiplier/enabled"].value then
				-- apply
				if not lastWalkSpeed then lastWalkSpeed = rawget(GameConstants, "BASE_WALKSPEED") end
				rawset(GameConstants, "BASE_WALKSPEED", lastWalkSpeed * base.features["misc/movement/speed_multiplier/mult"].value)
			else
				-- revert
				if lastWalkSpeed then
					rawset(GameConstants, "BASE_WALKSPEED", lastWalkSpeed)
					lastWalkSpeed = nil
				end
			end
		end
	...
	RunService.PreSimulation:Connect(LPH_NO_VIRTUALIZE(function()
		if not base.features["main/silent_aim/auto_shoot"].value then return end
		if not FighterController.LocalFighter then return end

		local target = Aiming.GetClosestTarget()
		if target then
...
do -- Skin changer
	if getconnections and islclosure and getconstants and setconstant then
		local CosmeticLibrary = require(ReplicatedStorage.Modules.CosmeticLibrary)
		local ItemLibrary = require(ReplicatedStorage.Modules.ItemLibrary)
		local Equipment = require(LocalPlayer.PlayerScripts.Modules.UserInterface.Equipment)
		local CosmeticSlot = require(LocalPlayer.PlayerScripts.Modules.CosmeticSlot)
		...
		local weaponCosmeticMap = {}

		local function SaveCosmeticMap()
			writefile(HOME_DIR .. "cosmetics/main.json", HttpService:JSONEncode(weaponCosmeticMap))
		end

		if isfile(HOME_DIR .. "cosmetics/main.json") then
			weaponCosmeticMap = HttpService:JSONDecode(readfile(HOME_DIR .. "cosmetics/main.json"))
			for _, v in next, weaponCosmeticMap do
				if v.Wrap == "None" or (type(v.Wrap) == "table" and (v.Wrap.Name == "None" or not v.Wrap.Name)) then
					v.Wrap = nil
				end
				if v.Skin == "None" then
					v.Skin = nil
				end
				if v.Charm == "None" then
					v.Charm = nil
				end
			end
			SaveCosmeticMap()
		...
		-- workaround to not hook global metamethod and have it work on shit executors
		if setidentity then
			setidentity(8)
		end
		ReplicatedStorage.Remotes.Data.EquipCosmetic.Parent = CoreGui
		local bindableEvent = Instance.new("BindableEvent", ReplicatedStorage.Remotes.Data)
		bindableEvent.Name = "EquipCosmetic"
		...
		bindableEvent.Event:Connect(function(...)
			...
			local inverted = args[4]

			if not weaponCosmeticMap[weaponName] then
				weaponCosmeticMap[weaponName] = {}
			end

			local cosmeticMap = weaponCosmeticMap[weaponName]
			if cosmeticType == "Wrap" then
				if cosmeticName ~= "None" then
					...
					cosmeticMap.Wrap = nil
				end
			elseif cosmeticType == "Skin" then
				if cosmeticName ~= nil then
					cosmeticMap.Skin = cosmeticName
				else
					-- print("set nil")
					cosmeticMap.Skin = nil
				end
			elseif cosmeticType == "Charm" then
				if cosmeticName ~= nil then
					cosmeticMap.Charm = cosmeticName
				else
					-- print("set nil 2")
					cosmeticMap.Charm = nil
				end
			end
			...
			task.defer(function()
				...
				PlayerDataController.CurrentData._value_changed_events["WeaponInventory"]:Fire()
				if setidentity then
					setidentity(8)
				end
			end)
		...
		base.features["skins/main/unlock_all"].changed:Connect(function(state)
			if state then
				for _, cosmeticName in next, CosmeticLibrary.CosmeticsAlphabetized do
					-- 2 lazy to figure out how the table is structured xD
					CosmeticInventory[cosmeticName] = setmetatable({}, {__index = function()
						return true
					end})
				end
			else
				for _, cosmeticName in next, CosmeticLibrary.CosmeticsAlphabetized do
					if getmetatable(CosmeticInventory[cosmeticName]) then
						CosmeticInventory[cosmeticName] = nil
					end
				end
			end
		end)
		...
		local o_GetWeaponData = PlayerDataController.GetWeaponData
		PlayerDataController.GetWeaponData = LPH_NO_VIRTUALIZE(function(self, itemName)
			local weaponData = o_GetWeaponData(self, itemName)
			if base.features["skins/main/unlock_all"].value and weaponCosmeticMap[itemName] then
				for name, data in next, weaponCosmeticMap[itemName] do
					if not originalWeaponData[name] then
						originalWeaponData[name] = weaponData[name]
					end
					weaponData[name] = data
				end
			else
				if weaponData then
					for name, data in weaponData do
						if originalWeaponData[name] then
							weaponData[name] = originalWeaponData[name]
						end
					end
				end
			end
		...
		local function ModifyInvertedButton()
			for _, connection in next, getconnections(Equipment.Customize.Frame.Container.Inverted.Button.MouseButton1Click) do
				if connection.Function and islclosure(connection.Function) then
					local idx = table.find(getconstants(connection.Function), "FireServer")
					if idx then
						setconstant(connection.Function, idx, "Fire")
					end
				end
			end
		end
		ModifyInvertedButton()

		local o_Init = PlayerDataController._Init
		PlayerDataController._Init = function(self)
			o_Init(self)
			ModifyInvertedButton()
		end

		local o_CosmeticSlot_new = CosmeticSlot.new
		CosmeticSlot.new = function(...)
			local cosmeticSlot = o_CosmeticSlot_new(...)
			task.defer(function()
				if not cosmeticSlot then return end
				local button = cosmeticSlot.Frame:WaitForChild("Button", 5)
				if not button then return end
				for _, connection in next, getconnections(button.MouseButton1Click) do
					if connection.Function and islclosure(connection.Function) then
						local idx = table.find(getconstants(connection.Function), "FireServer")
						if idx then
							setconstant(connection.Function, idx, "Fire")
		...
		-- oddly this breaks synapse z...
		-- PlayerDataController.CurrentData._value_changed_events["WeaponInventory"]:Fire()

		local Event = require(ReplicatedStorage.Modules.Event)

		local skinToItemNameMap = {}

		for skinName, data in next, CosmeticLibrary.Cosmetics do
			skinToItemNameMap[skinName] = data.ItemName
		end

		local o_ViewModel_Init = ClientViewModel._Init

		if setidentity then
			setidentity(8)
		end

		-- ClientViewModel._Init = function(self)
		-- 	local trueName = skinToItemNameMap[self.Name] or self.Name
		-- 	if weaponCosmeticMap[trueName] and (not not self.ClientItem.ClientFighter:Get("IsSpectating")) then
		-- 		self.ItemModel = Players.LocalPlayer.PlayerScripts.Assets.ViewModels:FindFirstChild(weaponCosmeticMap[trueName].Skin):Clone()
		-- 		self.Info = ItemLibrary.ViewModels[weaponCosmeticMap[trueName].Skin]
		-- 		self._root_part_offset_inverse = self.Info.RootPartOffset:Inverse()
		-- 		self._original_scale = self.ItemModel:GetScale()
		-- 		self._body_model = self.ItemModel:FindFirstChild("Body")
		-- 	end

		-- 	return o_ViewModel_Init(self)
		-- end

		-- local o_GetWrap = ClientViewModel.GetWrap
		-- ClientViewModel.GetWrap = function(self)
		-- 	local trueName = skinToItemNameMap[self.Name] or self.Name
		-- 	if weaponCosmeticMap[trueName] then
		-- 		return (not not self.ClientItem.ClientFighter:Get("IsSpectating") or not PlayerDataController:Get("Settings Wraps Disabled")) and weaponCosmeticMap[trueName].Wrap
		-- 	else
		-- 		return o_GetWrap(self)
		-- 	end
		-- end

		local Items = LocalPlayer.PlayerScripts.Modules.Items
		local ItemTypes = LocalPlayer.PlayerScripts.Modules.ItemTypes
		local ClientItem = require(LocalPlayer.PlayerScripts.Modules.ClientReplicatedClasses.ClientFighter.ClientItem)

		local LocalFighter = FighterController.LocalFighter
		local o_CreateItem = LocalFighter._CreateItem
		LocalFighter._CreateItem = function(self, v236, v237)
			-- table.foreach(v236[self:ToEnum("Data")], function(i, v)
			-- 	warn(self:FromEnum(i), v)
			-- end)
			if self.IsLocalPlayer then
				local data = v236[self:ToEnum("ViewModel")][self:ToEnum("Data")]
				local name = data[self:ToEnum("Name")]
				local trueName = skinToItemNameMap[name] or name
				if base.features["skins/main/unlock_all"].value then
					if weaponCosmeticMap[trueName] then
						if weaponCosmeticMap[trueName].Skin then
							data[self:ToEnum("Name")] = weaponCosmeticMap[trueName].Skin
						end
						if weaponCosmeticMap[trueName].Wrap then
							data[self:ToEnum("Wrap")] = weaponCosmeticMap[trueName].Wrap
						end
						if weaponCosmeticMap[trueName].Charm then
							data[self:ToEnum("Charm")] = weaponCosmeticMap[trueName].Charm
						end
					end
				else
					-- local weaponData = PlayerDataController:GetWeaponData(trueName)
					-- if weaponData then
					-- 	if weaponData.Skin then
					-- 		data[self:ToEnum("Name")] = weaponData.Skin
					-- 	else
					-- 		data[self:ToEnum("Name")] = nil
					-- 	end
					-- 	if weaponData.Wrap then
					-- 		data[self:ToEnum("Wrap")] = weaponData.Wrap.Name
					-- 	else
					-- 		data[self:ToEnum("Wrap")] = nil
					-- 	end
					-- 	if weaponData.Charm then
					-- 		data[self:ToEnum("Charm")] = weaponData.Charm
					-- 	else
					-- 		data[self:ToEnum("Charm")] = nil
					-- 	end
					-- end
				end
			end

			return o_CreateItem(self, v236, v237)
		end

		local function UpdateItems()
			local LocalFighter = FighterController.LocalFighter
			for idx, item in next, LocalFighter.Items do
				local viewmodel = item.ViewModel
				if not viewmodel.ClientItem.ClientFighter:Get("IsSpectating") then continue end
				local trueName = skinToItemNameMap[viewmodel.Name] or viewmodel.Name
				if not weaponCosmeticMap[trueName] then continue end

				local serializedItemData = {}
				for i, v in next, item.Data do
					if i ~= "IsAiming" and i ~= "FOVOffset" and i ~= "ItemIndex" then
						serializedItemData[LocalFighter:ToEnum(i)] = v
					end
				end

				local serializedViewModel = {}
				for i, v in next, item.ViewModel.Data do
					serializedViewModel[LocalFighter:ToEnum(i)] = v
				end

				local serializedTable = {
					[LocalFighter:ToEnum("Data")] = serializedItemData,
					[LocalFighter:ToEnum("ViewModel")] = {
						[LocalFighter:ToEnum("Data")] = serializedViewModel
					}
				}

				local itemIdx = item.Data.ItemIndex
				table.remove(LocalFighter.Items, itemIdx)
				LocalFighter.ItemRemoved:Fire(item)
				item:Destroy()

				if setidentity then
					setidentity(2)
				end
				LocalFighter:_CreateItem(serializedTable, itemIdx)
				if setidentity then
					setidentity(8)
				end
			end
		...
		base.features["skins/main/unlock_all"].changed:Connect(function()
			UpdateItems()
		end)
	end
end
